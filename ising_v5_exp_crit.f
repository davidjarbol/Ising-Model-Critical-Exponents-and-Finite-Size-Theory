	...
	parameter(N=128)     !DEBE SER PAR PARA CALCULAR BIEN funci¢n de correlaci¢n
	real*8 S(0:N+1,0:N+1)
	real*8 f(N)  !Funci¢n de correlaci¢n
	
	h=0.05d0  !Por defecto paso entre temperatura h=0.3 pero cerca de Tc veremos que lo reducimos
	...
	open(14,file='c_T.dat')
	open(15,file='chi_T.dat')
	open(16,file='f.dat')
...
!!!!!!!!!!!Calculo M, E, f  cada 100pMC
	...
	if (T.le.2.3d0) then
       ...
       !Cerca de temperatura cr¡tica (Tc=2.269) hay m s cambio de la funci¢n: necesitamos más datos --> h=0.1  paso temperatura
